...
task 'test', "", ['regenerate'], (done) ->
  ...
  await execute "glide up",               { cwd: './test' }, defer code, stderr, stdout
  await execute "go fmt ./generated/...", { cwd: './test' }, defer code, stderr, stdout
  await execute "go run ./runner.go",     { cwd: './test' }, defer code, stderr, stdout
  done();
