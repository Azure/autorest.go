    ...
    for (const op of values(group.operations)) {
      ...
      for (const param of values(aggregateParameters(op))) {
        ...
        if (param.implementation === ImplementationLocation.Client && param.schema.type !== SchemaType.Constant) {
          ...
          if (index === -1) {
            globals.push(new paramInfo(param.language.go!.name, param.schema.language.go!.name, true, param.required === true, (param.extensions ? (param.extensions!['x-ms-priority'] === 0 && param.extensions!['x-in'] === 'path') : false)));
          }
        }
      }
