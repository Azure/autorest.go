...
async function runAutorest(readmePath: string, extraOption: Array<string>) {
  const cmd =
        ...
        ' --version=3.9.6 --generate-sdk=false --testmodeler.generate-mock-test --testmodeler.generate-sdk-example --testmodeler.generate-scenario-test --testmodeler.generate-sdk-sample --use=@autorest/go@latest --use=' +
        path.join(`${__dirname}`, '..', '..') +
        ' ' +
        readmePath +
        ' ' +
