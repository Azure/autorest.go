          ...
          if (exampleValue.language?.go?.name === 'ProvisioningState') {
            if (
              exampleValue.schema.type !== SchemaType.SealedChoice ||
                        (<ChoiceSchema>exampleValue.schema).choices.filter((choice) => choice.value === 'Succeeded').length > 0
            ) {
              return 'Succeeded';
            } else {
      ...
      if (exampleValue.schema?.type === SchemaType.Array) {
        ...
        for (const parentsProp of parentsProps) {
          const isPolymophismValue =
                    ...
                    ((<ObjectSchema>parentsProp.schema).discriminatorValue !== undefined ||
                        (<ObjectSchema>parentsProp.schema).discriminator?.property.isDiscriminator === true);
