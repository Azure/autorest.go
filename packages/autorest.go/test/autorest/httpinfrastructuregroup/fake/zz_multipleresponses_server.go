...
func (m *MultipleResponsesServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		var res result
		if multipleResponsesServerTransportInterceptor != nil {
			res.resp, res.err, intercepted = multipleResponsesServerTransportInterceptor.Do(req)
		}
		if !intercepted {
			switch method {
...
func (m *MultipleResponsesServerTransport) dispatchGet200ModelA201ModelC404ModelDDefaultError200Valid(req *http.Request) (*http.Response, error) {
	...
	respContent := server.GetResponseContent(respr)
	if !slices.Contains([]int{http.StatusOK, http.StatusCreated, http.StatusNotFound}, respContent.HTTPStatus) {
...
// set this to conditionally intercept incoming requests to MultipleResponsesServerTransport
var multipleResponsesServerTransportInterceptor interface {
