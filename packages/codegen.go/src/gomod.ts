...
export async function generateGoModFile(codeModel: go.CodeModel, existingGoMod?: string): Promise<string> {
  if (!codeModel.options.module) {
    ...
    throw new DiagnosticError('--module and --module-version are required when go.mod exists');
  }
  ...
  if (codeModel.options.azcoreVersion) {
    ...
    if (!codeModel.options.azcoreVersion.match(/^\d+\.\d+\.\d+(?:-[a-zA-Z0-9_.-]+)?$/)) {
      throw new DiagnosticError(`azcore version ${version} must be in the format major.minor.patch[-beta.N]`);
    }
  ...
  if (match) {
    if (match.length < 2) {
      throw new DiagnosticError('returned matches were less than expected');
    }
