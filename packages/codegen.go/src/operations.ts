...
export async function generateOperations(codeModel: go.CodeModel): Promise<Array<OperationGroupContent>> {
  ...
  for (const client of codeModel.clients) {
    ...
    const isParamPointer = function(param: go.Parameter): boolean {
      ...
      return param.placement === 'flag' || param.placement === 'optional';
    };
...
function createProtocolRequest(azureARM: boolean, client: go.Client, method: go.Method | go.NextPageMethod, imports: ImportManager): string {
  ...
  const emitQueryParam = function (qp: go.QueryParameter, setter: string): string {
    let qpText = '';
    if (qp.location === 'method' && go.isClientSideDefault(qp.placement)) {
      qpText = emitClientSideDefault(qp, qp.placement, (name, val) => { return `\treqQP.Set(${name}, ${val})`; }, imports);
    } else if (go.isRequiredParameter(qp) || go.isLiteralParameter(qp) || (qp.location === 'client' && go.isClientSideDefault(qp.placement))) {
      qpText = `\t${setter}\n`;
  ...
  const emitHeaderSet = function (headerParam: go.HeaderParameter, prefix: string): string {
    if (headerParam.location === 'method' && go.isClientSideDefault(headerParam.placement)) {
      return emitClientSideDefault(headerParam, headerParam.placement, (name, val) => {
  ...
  for (const param of headerParams.sort((a: go.HeaderParameter, b: go.HeaderParameter) => { return helpers.sortAscending(a.headerName, b.headerName);})) {
    ...
    if (go.isRequiredParameter(param) || go.isLiteralParameter(param) || go.isClientSideDefault(param.placement)) {
