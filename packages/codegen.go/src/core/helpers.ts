...
export function getCreateRequestParametersSig(method: go.MethodType | go.NextPageMethod): string {
  ...
  for (const methodParam of methodParams) {
    let paramName = methodParam.kind !== 'paramGroup' && methodParam.isExactName ? methodParam.name : naming.uncapitalize(methodParam.name);
...
export function getCreateRequestParameters(method: go.MethodType, optionsParam?: string): string {
  ...
  for (let i = 0; i < methodParams.length; ++i) {
    ...
    if (optionsParam && i === methodParams.length - 1) {
      ...
      params.push(methodParam.kind !== 'paramGroup' && methodParam.isExactName ? methodParam.name : naming.uncapitalize(methodParam.name));
    }
  }
  return params.join(', ');
}
