...
func TestModelClient_Client(t *testing.T) {
	...
	resp, err := client.NewModelClient().Client(context.Background(), naminggroup.ClientModel{
...
func TestModelClient_Language(t *testing.T) {
	client, err := naminggroup.NewNamingClient(nil)
	require.NoError(t, err)
	resp, err := client.NewModelClient().Language(context.Background(), naminggroup.GoModel{
		DefaultName: to.Ptr(true),
	}, nil)
	require.NoError(t, err)
	require.Zero(t, resp)
