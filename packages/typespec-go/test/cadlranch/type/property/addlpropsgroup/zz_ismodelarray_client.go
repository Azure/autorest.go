// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See License.txt in the project root for license information.
// Code generated by Microsoft (R) Go Code Generator. DO NOT EDIT.

package addlpropsgroup

import (
	"context"
	"github.com/Azure/azure-sdk-for-go/sdk/azcore"
	"github.com/Azure/azure-sdk-for-go/sdk/azcore/policy"
	"github.com/Azure/azure-sdk-for-go/sdk/azcore/runtime"
	"net/http"
)

// IsModelArrayClient contains the methods for the Type.Property.AdditionalProperties namespace.
// Don't use this type directly, use [AdditionalPropertiesClient.NewIsModelArrayClient] instead.
type IsModelArrayClient struct {
	internal *azcore.Client
}

// Get - Get call
//   - options - IsModelArrayClientGetOptions contains the optional parameters for the IsModelArrayClient.Get method.
func (client *IsModelArrayClient) Get(ctx context.Context, options *IsModelArrayClientGetOptions) (IsModelArrayClientGetResponse, error) {
	var err error
	req, err := client.getCreateRequest(ctx, options)
	if err != nil {
		return IsModelArrayClientGetResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return IsModelArrayClientGetResponse{}, err
	}
	if !runtime.HasStatusCode(httpResp, http.StatusOK) {
		err = runtime.NewResponseError(httpResp)
		return IsModelArrayClientGetResponse{}, err
	}
	resp, err := client.getHandleResponse(httpResp)
	return resp, err
}

// getCreateRequest creates the Get request.
func (client *IsModelArrayClient) getCreateRequest(ctx context.Context, options *IsModelArrayClientGetOptions) (*policy.Request, error) {
	urlPath := "/type/property/additionalProperties/isRecordModelArray"
	req, err := runtime.NewRequest(ctx, http.MethodGet, runtime.JoinPaths(host, urlPath))
	if err != nil {
		return nil, err
	}
	req.Raw().Header["Accept"] = []string{"application/json"}
	return req, nil
}

// getHandleResponse handles the Get response.
func (client *IsModelArrayClient) getHandleResponse(resp *http.Response) (IsModelArrayClientGetResponse, error) {
	result := IsModelArrayClientGetResponse{}
	if err := runtime.UnmarshalAsJSON(resp, &result.IsModelArrayAdditionalProperties); err != nil {
		return IsModelArrayClientGetResponse{}, err
	}
	return result, nil
}

// Put - Put operation
//   - body - body
//   - options - IsModelArrayClientPutOptions contains the optional parameters for the IsModelArrayClient.Put method.
func (client *IsModelArrayClient) Put(ctx context.Context, body IsModelArrayAdditionalProperties, options *IsModelArrayClientPutOptions) (IsModelArrayClientPutResponse, error) {
	var err error
	req, err := client.putCreateRequest(ctx, body, options)
	if err != nil {
		return IsModelArrayClientPutResponse{}, err
	}
	httpResp, err := client.internal.Pipeline().Do(req)
	if err != nil {
		return IsModelArrayClientPutResponse{}, err
	}
	if !runtime.HasStatusCode(httpResp, http.StatusNoContent) {
		err = runtime.NewResponseError(httpResp)
		return IsModelArrayClientPutResponse{}, err
	}
	return IsModelArrayClientPutResponse{}, nil
}

// putCreateRequest creates the Put request.
func (client *IsModelArrayClient) putCreateRequest(ctx context.Context, body IsModelArrayAdditionalProperties, options *IsModelArrayClientPutOptions) (*policy.Request, error) {
	urlPath := "/type/property/additionalProperties/isRecordModelArray"
	req, err := runtime.NewRequest(ctx, http.MethodPut, runtime.JoinPaths(host, urlPath))
	if err != nil {
		return nil, err
	}
	req.Raw().Header["Content-Type"] = []string{"application/json"}
	if err := runtime.MarshalAsJSON(req, body); err != nil {
		return nil, err
	}
	return req, nil
}
