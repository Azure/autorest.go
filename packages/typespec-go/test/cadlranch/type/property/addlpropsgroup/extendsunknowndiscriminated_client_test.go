...
func TestExtendsUnknownDiscriminatedClient_Get(t *testing.T) {
	client, err := addlpropsgroup.NewExtendsUnknownDiscriminatedClient(nil)
...
func TestExtendsUnknownDiscriminatedClient_Put(t *testing.T) {
	client, err := addlpropsgroup.NewExtendsUnknownDiscriminatedClient(nil)
	require.NoError(t, err)
	resp, err := client.Put(context.Background(), &addlpropsgroup.ExtendsUnknownAdditionalPropertiesDiscriminatedDerived{
		Index: to.Ptr[int32](314),
		Kind:  to.Ptr("derived"),
		Name:  to.Ptr("Derived"),
		AdditionalProperties: map[string]any{
