...
func TestXMLModelWithNamespaceOnPropertiesValueClient_Get(t *testing.T) {
	client, err := xmlgroup.NewXMLClientWithNoCredential("http://localhost:3000", nil)
	require.NoError(t, err)
	resp, err := client.NewXMLModelWithNamespaceOnPropertiesValueClient().Get(context.Background(), nil)
	require.NoError(t, err)
	require.EqualValues(t, xmlgroup.ModelWithNamespaceOnProperties{
		ID:     to.Ptr[int32](123),
		Title:  to.Ptr("The Great Gatsby"),
		Author: to.Ptr("F. Scott Fitzgerald"),
	}, resp.ModelWithNamespaceOnProperties)
}
