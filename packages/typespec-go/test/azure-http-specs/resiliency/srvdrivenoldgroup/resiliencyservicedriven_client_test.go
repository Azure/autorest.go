...
func TestResiliencyServiceDrivenClientv1_FromNone(t *testing.T) {
	client, err := srvdrivenoldgroup.NewResiliencyServiceDrivenClientWithNoCredential("http://localhost:3000", "v1", nil)
...
func TestResiliencyServiceDrivenClientv2_FromNone(t *testing.T) {
	client, err := srvdrivenoldgroup.NewResiliencyServiceDrivenClientWithNoCredential("http://localhost:3000", "v2", nil)
	require.NoError(t, err)
	resp, err := client.FromNone(context.Background(), nil)
	require.NoError(t, err)
	require.True(t, resp.Success)
