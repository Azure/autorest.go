...
func TestIndividuallyParentNestedWithParamAliasClient_ViaParent_WithAliasedName(t *testing.T) {
	parent, err := clientinitindividuallyparentgroup.NewIndividuallyParentClientWithNoCredential("http://localhost:3000", nil)
	require.NoError(t, err)
	client := parent.NewIndividuallyParentIndividuallyParentNestedWithParamAliasClient("sample-blob")
	resp, err := client.WithAliasedName(context.Background(), nil)
	require.NoError(t, err)
	require.Zero(t, resp)
}
