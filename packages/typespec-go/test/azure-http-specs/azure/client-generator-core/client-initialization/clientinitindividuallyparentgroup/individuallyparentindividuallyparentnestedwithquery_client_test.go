...
func TestIndividuallyParentNestedWithQueryClient_ViaParent_WithQuery(t *testing.T) {
	parent, err := clientinitindividuallyparentgroup.NewIndividuallyParentClientWithNoCredential("http://localhost:3000", nil)
	require.NoError(t, err)
	client := parent.NewIndividuallyParentIndividuallyParentNestedWithQueryClient("test-blob")
	resp, err := client.WithQuery(context.Background(), &clientinitindividuallyparentgroup.IndividuallyParentIndividuallyParentNestedWithQueryClientWithQueryOptions{
		Format: to.Ptr("text"),
	})
	require.NoError(t, err)
