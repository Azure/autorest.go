...
func TestIndividuallyParentNestedWithHeaderClient_ViaParent_WithQuery(t *testing.T) {
	parent, err := clientinitindividuallyparentgroup.NewIndividuallyParentClientWithNoCredential("http://localhost:3000", nil)
	require.NoError(t, err)
	client := parent.NewIndividuallyParentIndividuallyParentNestedWithHeaderClient("test-name-value")
	resp, err := client.WithQuery(context.Background(), &clientinitindividuallyparentgroup.IndividuallyParentIndividuallyParentNestedWithHeaderClientWithQueryOptions{
		Format: to.Ptr("text"),
	})
	require.NoError(t, err)
