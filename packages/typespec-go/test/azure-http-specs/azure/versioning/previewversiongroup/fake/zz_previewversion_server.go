...
type PreviewVersionServer struct {
	...
	// UpdateWidgetColor is the fake for method PreviewVersionClient.UpdateWidgetColor
	// HTTP status codes to indicate success: http.StatusOK, http.StatusNotFound
...
// The returned PreviewVersionServerTransport instance is connected to an instance of previewversiongroup.PreviewVersionClient via the
...
// PreviewVersionServerTransport connects instances of previewversiongroup.PreviewVersionClient to instances of PreviewVersionServer.
...
func (p *PreviewVersionServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "PreviewVersionClient.UpdateWidgetColor":
				res.resp, res.err = p.dispatchUpdateWidgetColor(req)
			default:
...
func (p *PreviewVersionServerTransport) dispatchUpdateWidgetColor(req *http.Request) (*http.Response, error) {
	...
	body, err := server.UnmarshalRequestAsJSON[previewversiongroup.UpdateWidgetColorRequest](req)
	if err != nil {
		return nil, err
	}
	idParam, err := url.PathUnescape(matches[regex.SubexpIndex("id")])
	...
	respr, errRespr := p.srv.UpdateWidgetColor(req.Context(), idParam, body, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
