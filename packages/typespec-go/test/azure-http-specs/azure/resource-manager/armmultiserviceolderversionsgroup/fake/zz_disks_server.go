...
// DisksServer is a fake server for instances of the armmultiserviceolderversionsgroup.DisksClient type.
type DisksServer struct {
	// BeginCreateOrUpdate is the fake for method DisksClient.BeginCreateOrUpdate
	// HTTP status codes to indicate success: http.StatusOK, http.StatusCreated
	BeginCreateOrUpdate func(ctx context.Context, resourceGroupName string, diskName string, resource armmultiserviceolderversionsgroup.Disk, options *armmultiserviceolderversionsgroup.DisksClientBeginCreateOrUpdateOptions) (resp azfake.PollerResponder[armmultiserviceolderversionsgroup.DisksClientCreateOrUpdateResponse], errResp azfake.ErrorResponder)

	// Get is the fake for method DisksClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceGroupName string, diskName string, options *armmultiserviceolderversionsgroup.DisksClientGetOptions) (resp azfake.Responder[armmultiserviceolderversionsgroup.DisksClientGetResponse], errResp azfake.ErrorResponder)
}

// NewDisksServerTransport creates a new instance of DisksServerTransport with the provided implementation.
// The returned DisksServerTransport instance is connected to an instance of armmultiserviceolderversionsgroup.DisksClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewDisksServerTransport(srv *DisksServer) *DisksServerTransport {
	return &DisksServerTransport{
		srv:                 srv,
		beginCreateOrUpdate: newTracker[azfake.PollerResponder[armmultiserviceolderversionsgroup.DisksClientCreateOrUpdateResponse]](),
	}
}

// DisksServerTransport connects instances of armmultiserviceolderversionsgroup.DisksClient to instances of DisksServer.
// Don't use this type directly, use NewDisksServerTransport instead.
type DisksServerTransport struct {
	srv                 *DisksServer
	beginCreateOrUpdate *tracker[azfake.PollerResponder[armmultiserviceolderversionsgroup.DisksClientCreateOrUpdateResponse]]
}
...
func (d *DisksServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Compute/disksOld/(?P<diskName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armmultiserviceolderversionsgroup.Disk](req)
		if err != nil {
			return nil, err
		}
