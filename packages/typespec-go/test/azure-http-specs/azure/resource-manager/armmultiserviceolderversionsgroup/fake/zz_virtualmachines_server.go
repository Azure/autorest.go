...
// VirtualMachinesServer is a fake server for instances of the armmultiserviceolderversionsgroup.VirtualMachinesClient type.
type VirtualMachinesServer struct {
	...
	BeginCreateOrUpdate func(ctx context.Context, resourceGroupName string, vmName string, resource armmultiserviceolderversionsgroup.VirtualMachine, options *armmultiserviceolderversionsgroup.VirtualMachinesClientBeginCreateOrUpdateOptions) (resp azfake.PollerResponder[armmultiserviceolderversionsgroup.VirtualMachinesClientCreateOrUpdateResponse], errResp azfake.ErrorResponder)

	// Get is the fake for method VirtualMachinesClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceGroupName string, vmName string, options *armmultiserviceolderversionsgroup.VirtualMachinesClientGetOptions) (resp azfake.Responder[armmultiserviceolderversionsgroup.VirtualMachinesClientGetResponse], errResp azfake.ErrorResponder)
}
...
// The returned VirtualMachinesServerTransport instance is connected to an instance of armmultiserviceolderversionsgroup.VirtualMachinesClient via the
...
func NewVirtualMachinesServerTransport(srv *VirtualMachinesServer) *VirtualMachinesServerTransport {
	return &VirtualMachinesServerTransport{
		srv:                 srv,
		beginCreateOrUpdate: newTracker[azfake.PollerResponder[armmultiserviceolderversionsgroup.VirtualMachinesClientCreateOrUpdateResponse]](),
	}
}

// VirtualMachinesServerTransport connects instances of armmultiserviceolderversionsgroup.VirtualMachinesClient to instances of VirtualMachinesServer.
...
type VirtualMachinesServerTransport struct {
	...
	beginCreateOrUpdate *tracker[azfake.PollerResponder[armmultiserviceolderversionsgroup.VirtualMachinesClientCreateOrUpdateResponse]]
}
...
func (v *VirtualMachinesServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Compute/virtualMachinesOld/(?P<vmName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armmultiserviceolderversionsgroup.VirtualMachine](req)
		if err != nil {
			return nil, err
		}
