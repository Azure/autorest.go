...
// VirtualMachinesServer is a fake server for instances of the armmultipleservicegroup.VirtualMachinesClient type.
type VirtualMachinesServer struct {
	...
	BeginCreateOrUpdate func(ctx context.Context, resourceGroupName string, vmName string, resource armmultipleservicegroup.VirtualMachine, options *armmultipleservicegroup.VirtualMachinesClientBeginCreateOrUpdateOptions) (resp azfake.PollerResponder[armmultipleservicegroup.VirtualMachinesClientCreateOrUpdateResponse], errResp azfake.ErrorResponder)

	// Get is the fake for method VirtualMachinesClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceGroupName string, vmName string, options *armmultipleservicegroup.VirtualMachinesClientGetOptions) (resp azfake.Responder[armmultipleservicegroup.VirtualMachinesClientGetResponse], errResp azfake.ErrorResponder)
}
...
// The returned VirtualMachinesServerTransport instance is connected to an instance of armmultipleservicegroup.VirtualMachinesClient via the
...
func NewVirtualMachinesServerTransport(srv *VirtualMachinesServer) *VirtualMachinesServerTransport {
	return &VirtualMachinesServerTransport{
		srv:                 srv,
		beginCreateOrUpdate: newTracker[azfake.PollerResponder[armmultipleservicegroup.VirtualMachinesClientCreateOrUpdateResponse]](),
	}
}

// VirtualMachinesServerTransport connects instances of armmultipleservicegroup.VirtualMachinesClient to instances of VirtualMachinesServer.
...
type VirtualMachinesServerTransport struct {
	...
	beginCreateOrUpdate *tracker[azfake.PollerResponder[armmultipleservicegroup.VirtualMachinesClientCreateOrUpdateResponse]]
}
...
func (v *VirtualMachinesServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Compute/virtualMachines/(?P<vmName>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[armmultipleservicegroup.VirtualMachine](req)
		if err != nil {
			return nil, err
		}
