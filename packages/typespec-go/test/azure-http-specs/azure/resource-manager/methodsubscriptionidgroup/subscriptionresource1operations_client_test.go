...
func TestSubscriptionResource1OperationsClient_CRUD(t *testing.T) {
	...
	client, err := methodsubscriptionidgroup.NewSubscriptionResource1OperationsClient(subscriptionID, cred, nil)
	require.NoError(t, err)
	require.NotNil(t, client)

	resourceName := "test-resource1"
	resource := methodsubscriptionidgroup.SubscriptionResource1{
	...
	require.NoError(t, err)
	require.NotNil(t, putResp.SubscriptionResource1)

	// Get
	getResp, err := client.Get(context.Background(), resourceName, nil)
	require.NoError(t, err)
	require.NotNil(t, getResp.SubscriptionResource1)
