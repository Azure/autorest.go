...
func (r *ReplicasServerTransport) dispatchNewListByParentPager(req *http.Request) (*http.Response, error) {
	...
	if newListByParentPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.DocumentDB/mongoClusters/(?P<mongoClusterName>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)/replicas`
