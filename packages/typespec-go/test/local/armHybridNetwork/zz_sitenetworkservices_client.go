...
// BeginCancelOperation - Cancels an ongoing long-running PUT operation for the specified Site Network Service resource. Other
// operations are not supported for cancellation at this time.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2025-03-30
//   - parameters - The SiteNetworkService detail and an optional operation which defaults to 'Put'.
//   - options - SiteNetworkServicesClientBeginCancelOperationOptions contains the optional parameters for the SiteNetworkServicesClient.BeginCancelOperation
//     method.
func (client *SiteNetworkServicesClient) BeginCancelOperation(ctx context.Context, parameters CancelInformation, options *SiteNetworkServicesClientBeginCancelOperationOptions) (*runtime.Poller[SiteNetworkServicesClientCancelOperationResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.cancelOperation(ctx, parameters, options)
		if err != nil {
			return nil, err
		}
		poller, err := runtime.NewPoller(resp, client.internal.Pipeline(), &runtime.NewPollerOptions[SiteNetworkServicesClientCancelOperationResponse]{
			Tracer: client.internal.Tracer(),
		})
		return poller, err
	} else {
		return runtime.NewPollerFromResumeToken(options.ResumeToken, client.internal.Pipeline(), &runtime.NewPollerFromResumeTokenOptions[SiteNetworkServicesClientCancelOperationResponse]{
			Tracer: client.internal.Tracer(),
		})
	}
}

// CancelOperation - Cancels an ongoing long-running PUT operation for the specified Site Network Service resource. Other
// operations are not supported for cancellation at this time.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2025-03-30
func (client *SiteNetworkServicesClient) cancelOperation(ctx context.Context, parameters CancelInformation, options *SiteNetworkServicesClientBeginCancelOperationOptions) (*http.Response, error) {
	var err error
	const operationName = "SiteNetworkServicesClient.BeginCancelOperation"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.cancelOperationCreateRequest(ctx, parameters, options)
	if err != nil {
		return nil, err
	}
...
// cancelOperationCreateRequest creates the CancelOperation request.
func (client *SiteNetworkServicesClient) cancelOperationCreateRequest(ctx context.Context, parameters CancelInformation, _ *SiteNetworkServicesClientBeginCancelOperationOptions) (*policy.Request, error) {
	urlPath := "/subscriptions/{subscriptionId}/providers/Microsoft.HybridNetwork/cancelSiteNetworkServiceOperation"
