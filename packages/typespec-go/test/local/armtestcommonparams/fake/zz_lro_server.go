...
// LROServer is a fake server for instances of the armtestcommonparams.LROClient type.
type LROServer struct {
	...
	BeginOkResponseWithAsyncHeader func(ctx context.Context, resourceGroupName string, lroModelName string, resource armtestcommonparams.TestLROModel, options *armtestcommonparams.LROClientBeginOkResponseWithAsyncHeaderOptions) (resp azfake.PollerResponder[armtestcommonparams.LROClientOkResponseWithAsyncHeaderResponse], errResp azfake.ErrorResponder)

	// BeginScalarResult is the fake for method LROClient.BeginScalarResult
	// HTTP status codes to indicate success: http.StatusOK, http.StatusAccepted
	BeginScalarResult func(ctx context.Context, resourceGroupName string, lroModelName string, body armtestcommonparams.ActionRequest, options *armtestcommonparams.LROClientBeginScalarResultOptions) (resp azfake.PollerResponder[armtestcommonparams.LROClientScalarResultResponse], errResp azfake.ErrorResponder)
}

// NewLROServerTransport creates a new instance of LROServerTransport with the provided implementation.
// The returned LROServerTransport instance is connected to an instance of armtestcommonparams.LROClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewLROServerTransport(srv *LROServer) *LROServerTransport {
	return &LROServerTransport{
		srv:                            srv,
		beginOkResponseWithAsyncHeader: newTracker[azfake.PollerResponder[armtestcommonparams.LROClientOkResponseWithAsyncHeaderResponse]](),
		beginScalarResult:              newTracker[azfake.PollerResponder[armtestcommonparams.LROClientScalarResultResponse]](),
	}
}

// LROServerTransport connects instances of armtestcommonparams.LROClient to instances of LROServer.
// Don't use this type directly, use NewLROServerTransport instead.
type LROServerTransport struct {
	srv                            *LROServer
	beginOkResponseWithAsyncHeader *tracker[azfake.PollerResponder[armtestcommonparams.LROClientOkResponseWithAsyncHeaderResponse]]
	beginScalarResult              *tracker[azfake.PollerResponder[armtestcommonparams.LROClientScalarResultResponse]]
}
...
func (l *LROServerTransport) dispatchBeginOkResponseWithAsyncHeader(req *http.Request) (*http.Response, error) {
	...
	if beginOkResponseWithAsyncHeader == nil {
		...
		body, err := server.UnmarshalRequestAsJSON[armtestcommonparams.TestLROModel](req)
		if err != nil {
			return nil, err
		}
...
func (l *LROServerTransport) dispatchBeginScalarResult(req *http.Request) (*http.Response, error) {
	...
	if beginScalarResult == nil {
		...
		body, err := server.UnmarshalRequestAsJSON[armtestcommonparams.ActionRequest](req)
		if err != nil {
			return nil, err
		}
		resourceGroupNameParam, err := url.PathUnescape(matches[regex.SubexpIndex("resourceGroupName")])
