...
type OracleSubscriptionsServer struct {
	...
	// ListCloudAccountDetails is the fake for method OracleSubscriptionsClient.ListCloudAccountDetails
	// HTTP status codes to indicate success: http.StatusOK
	ListCloudAccountDetails func(ctx context.Context, options *armoracledatabase.OracleSubscriptionsClientListCloudAccountDetailsOptions) (resp azfake.Responder[armoracledatabase.OracleSubscriptionsClientListCloudAccountDetailsResponse], errResp azfake.ErrorResponder)
...
func (o *OracleSubscriptionsServerTransport) dispatchToMethodFake(req *http.Request, method string) (*http.Response, error) {
	...
	go func() {
		...
		if !intercepted {
			...
			case "OracleSubscriptionsClient.ListCloudAccountDetails":
				res.resp, res.err = o.dispatchListCloudAccountDetails(req)
...
func (o *OracleSubscriptionsServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Oracle\.Database/oracleSubscriptions/default`
...
func (o *OracleSubscriptionsServerTransport) dispatchNewListBySubscriptionPager(req *http.Request) (*http.Response, error) {
	...
	if newListBySubscriptionPager == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Oracle\.Database/oracleSubscriptions`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
...
func (o *OracleSubscriptionsServerTransport) dispatchListCloudAccountDetails(req *http.Request) (*http.Response, error) {
	if o.srv.ListCloudAccountDetails == nil {
		return nil, &nonRetriableError{errors.New("fake for method ListCloudAccountDetails not implemented")}
	}
	const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Oracle\.Database/oracleSubscriptions/default/listCloudAccountDetails`
	regex := regexp.MustCompile(regexStr)
	matches := regex.FindStringSubmatch(req.URL.EscapedPath())
	if len(matches) < 2 {
		return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
	}
	respr, errRespr := o.srv.ListCloudAccountDetails(req.Context(), nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
	...
	resp, err := server.MarshalResponseAsJSON(respContent, server.GetResponse(respr).CloudAccountDetails, req)
	if err != nil {
		return nil, err
	}
	return resp, nil
...
func (o *OracleSubscriptionsServerTransport) dispatchBeginListSaasSubscriptionDetails(req *http.Request) (*http.Response, error) {
	...
	if beginListSaasSubscriptionDetails == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-.0-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Oracle\.Database/oracleSubscriptions/default/listSaasSubscriptionDetails`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
