...
export class clientAdapter {
  ...
  private adaptMethodParameter(param: tcgc.SdkBodyParameter | tcgc.SdkHeaderParameter | tcgc.SdkPathParameter | tcgc.SdkQueryParameter | tcgc.SdkCookieParameter, verb: go.HTTPMethod): go.MethodParameter {
    if (param.isApiVersionParam) {
      ...
      const paramType = param.clientDefaultValue ? new go.Literal(new go.String(), param.clientDefaultValue) : new go.String();
      const paramStyle = param.clientDefaultValue ? 'literal' : param.optional ? 'optional' : 'required';
      ...
      switch (param.kind) {
        case 'header':
          apiVersionParam = new go.HeaderScalarParameter('apiVersion', param.serializedName, paramType, paramStyle, true, paramLoc);
          break;
        case 'path':
          apiVersionParam = new go.PathScalarParameter('apiVersion', param.serializedName, true, paramType, paramStyle, true, paramLoc);
          break;
        case 'query':
          apiVersionParam = new go.QueryScalarParameter('apiVersion', param.serializedName, true, paramType, paramStyle, true, paramLoc);
          break;
        default:
