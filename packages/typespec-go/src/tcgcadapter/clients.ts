...
export class clientAdapter {
  ...
  private adaptURIParam(sdkParam: tcgc.SdkEndpointParameter | tcgc.SdkPathParameter): go.URIParameter {
    ...
    return new go.URIParameter(sdkParam.name, sdkParam.serializedName ? sdkParam.serializedName : sdkParam.name, paramType,
      this.adaptParameterPlacement(sdkParam), isTypePassedByValue(sdkParam.type) || !sdkParam.optional, 'client');
  }
  ...
  private adaptMethodParameters(sdkMethod: tcgc.SdkServiceMethod<tcgc.SdkHttpOperation>, method: go.Method | go.NextPageMethod) {
    ...
    for (const param of sdkMethod.parameters) {
      ...
      if (opParam.kind === 'body' && opParam.type.kind === 'model' && opParam.type.kind !== param.type.kind) {
        const placement = this.adaptParameterPlacement(opParam);
        ...
        switch (contentType) {
          ...
          case 'XML': {
            ...
            adaptedParam = new go.PartialBodyParameter(param.name, serializedName, contentType, this.ta.getPossibleType(param.type, true, true), placement, byVal);
            break;
          }
          case 'binary':
            if (opParam.defaultContentType.match(/multipart/i)) {
              adaptedParam = new go.MultipartFormBodyParameter(param.name, this.ta.getReadSeekCloser(false), placement, byVal);
            } else {
              adaptedParam = new go.BodyParameter(param.name, contentType, `"${opParam.defaultContentType}"`, this.ta.getReadSeekCloser(false), placement, byVal);
            }
            break;
          default:
  ...
  private adaptMethodParameter(param: tcgc.SdkBodyParameter | tcgc.SdkHeaderParameter | tcgc.SdkPathParameter | tcgc.SdkQueryParameter, optionalGroup?: go.ParameterGroup): go.Parameter {
    ...
    const placement = this.adaptParameterPlacement(param);
    const byVal = isTypePassedByValue(param.type);

    if (param.kind === 'body') {
      // TODO: form data? (non-multipart)
      if (param.defaultContentType.match(/multipart/i)) {
        adaptedParam = new go.MultipartFormBodyParameter(paramName, this.ta.getPossibleType(param.type, false, true), placement, byVal);
      } else {
        ...
        adaptedParam = new go.BodyParameter(paramName, contentType, `"${param.defaultContentType}"`, bodyType, placement, byVal);
      }
      ...
      if (param.collectionFormat) {
        ...
        adaptedParam = new go.HeaderCollectionParameter(paramName, param.serializedName, type, param.collectionFormat, placement, byVal, location);
      } else {
        adaptedParam = new go.HeaderParameter(paramName, param.serializedName, this.adaptHeaderType(param.type, true), placement, byVal, location);
      }
    } else if (param.kind === 'path') {
      adaptedParam = new go.PathParameter(paramName, param.serializedName, param.urlEncode, this.adaptPathParameterType(param.type), placement, byVal, location);
    } else {
      if (param.collectionFormat) {
        const type = this.ta.getPossibleType(param.type, true, false);
        if (!go.isSliceType(type)) {
        ...
        adaptedParam = new go.QueryCollectionParameter(paramName, param.serializedName, true, type, param.collectionFormat, placement, byVal, location);
      } else {
        // TODO: unencoded query param
        adaptedParam = new go.QueryParameter(paramName, param.serializedName, true, this.adaptQueryParameterType(param.type), placement, byVal, location);
      }
    }
    ...
    } else if (placement !== 'required' && placement !== 'literal') {
  ...
  private adaptParameterGroup(paramGroup: go.ParameterGroup): go.StructType {
    ...
    for (const param of paramGroup.params) {
      if (param.placement === 'literal') {
        continue;
      }
      let byValue = param.placement === 'required' || (param.location === 'client' && go.isClientSideDefault(param.placement));
  ...
  private adaptParameterPlacement(param: tcgc.SdkBodyParameter | tcgc.SdkEndpointParameter | tcgc.SdkHeaderParameter | tcgc.SdkPathParameter | tcgc.SdkQueryParameter): go.ParameterPlacement {
