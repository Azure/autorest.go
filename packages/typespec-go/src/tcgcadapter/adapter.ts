...
export async function tcgcToGoCodeModel(context: EmitContext<GoEmitterOptions>): Promise<go.CodeModel> {
  ...
  const options = new go.Options(headerText, context.options['generate-fakes'] === true, context.options['inject-spans'] === true, context.options['disallow-unknown-fields'] === true, context.options['generate-examples'] === true);
  ...
  if (context.options.module && context.options['module-version']) {
    ...
    throw new DiagnosticError('--module and --module-version must both or neither be set');
  }
...
function fixStutteringTypeNames(sdkPackage: tcgc.SdkPackage<tcgc.SdkHttpOperation>, codeModel: go.CodeModel, options: GoEmitterOptions): void {
  ...
  if (collisions.length > 0) {
    throw new DiagnosticError(collisions.join('\n'));
  }
}
