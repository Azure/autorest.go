...
// ProductResultValueWithXMSClientNameIterator provides access to a complete listing of Product values.
type ProductResultValueWithXMSClientNameIterator struct {
	i    int
	page ProductResultValueWithXMSClientNamePage
}

// NextWithContext advances to the next value.  If there was an error making
// the request the iterator does not advance and the error is returned.
func (iter *ProductResultValueWithXMSClientNameIterator) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ProductResultValueWithXMSClientNameIterator.NextWithContext")
		defer func() {
			sc := -1
			if iter.Response().Response.Response != nil {
				sc = iter.Response().Response.Response.StatusCode
...
// the request the iterator does not advance and the error is returned.
// Deprecated: Use NextWithContext() instead.
func (iter *ProductResultValueWithXMSClientNameIterator) Next() error {
	return iter.NextWithContext(context.Background())
}

// NotDone returns true if the enumeration should be started or is not yet complete.
func (iter ProductResultValueWithXMSClientNameIterator) NotDone() bool {
	return iter.page.NotDone() && iter.i < len(iter.page.Values())
}

// Response returns the raw server response from the last page request.
func (iter ProductResultValueWithXMSClientNameIterator) Response() ProductResultValueWithXMSClientName {
	return iter.page.Response()
}

// Value returns the current value or a zero-initialized value if the
// iterator has advanced beyond the end of the collection.
func (iter ProductResultValueWithXMSClientNameIterator) Value() Product {
	if !iter.page.NotDone() {
		return Product{}
	}
	return iter.page.Values()[iter.i]
...
// Creates a new instance of the ProductResultValueWithXMSClientNameIterator type.
func NewProductResultValueWithXMSClientNameIterator(page ProductResultValueWithXMSClientNamePage) ProductResultValueWithXMSClientNameIterator {
	return ProductResultValueWithXMSClientNameIterator{page: page}
}

// IsEmpty returns true if the ListResult contains no values.
func (prvwxcn ProductResultValueWithXMSClientName) IsEmpty() bool {
	return prvwxcn.Values == nil || len(*prvwxcn.Values) == 0
}

// productResultValueWithXMSClientNamePreparer prepares a request to retrieve the next set of results.
// It returns nil if no more results exist.
func (prvwxcn ProductResultValueWithXMSClientName) productResultValueWithXMSClientNamePreparer(ctx context.Context) (*http.Request, error) {
	if prvwxcn.NextLink == nil || len(to.String(prvwxcn.NextLink)) < 1 {
		return nil, nil
	}
	return autorest.Prepare((&http.Request{}).WithContext(ctx),
		autorest.AsJSON(),
		autorest.AsGet(),
		autorest.WithBaseURL(to.String(prvwxcn.NextLink)))
}

// ProductResultValueWithXMSClientNamePage contains a page of Product values.
type ProductResultValueWithXMSClientNamePage struct {
	fn      func(context.Context, ProductResultValueWithXMSClientName) (ProductResultValueWithXMSClientName, error)
	prvwxcn ProductResultValueWithXMSClientName
}

// NextWithContext advances to the next page of values.  If there was an error making
// the request the page does not advance and the error is returned.
func (page *ProductResultValueWithXMSClientNamePage) NextWithContext(ctx context.Context) (err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/ProductResultValueWithXMSClientNamePage.NextWithContext")
		defer func() {
			sc := -1
			if page.Response().Response.Response != nil {
				sc = page.Response().Response.Response.StatusCode
	...
	next, err := page.fn(ctx, page.prvwxcn)
	if err != nil {
		return err
	}
	page.prvwxcn = next
...
// the request the page does not advance and the error is returned.
// Deprecated: Use NextWithContext() instead.
func (page *ProductResultValueWithXMSClientNamePage) Next() error {
	return page.NextWithContext(context.Background())
}

// NotDone returns true if the page enumeration should be started or is not yet complete.
func (page ProductResultValueWithXMSClientNamePage) NotDone() bool {
	return !page.prvwxcn.IsEmpty()
}

// Response returns the raw server response from the last page request.
func (page ProductResultValueWithXMSClientNamePage) Response() ProductResultValueWithXMSClientName {
	return page.prvwxcn
}

// Values returns the slice of values for the current page or nil if there are no values.
func (page ProductResultValueWithXMSClientNamePage) Values() []Product {
	if page.prvwxcn.IsEmpty() {
		return nil
	}
	return *page.prvwxcn.Values
}

// Creates a new instance of the ProductResultValueWithXMSClientNamePage type.
func NewProductResultValueWithXMSClientNamePage(getNextPage func(context.Context, ProductResultValueWithXMSClientName) (ProductResultValueWithXMSClientName, error)) ProductResultValueWithXMSClientNamePage {
	return ProductResultValueWithXMSClientNamePage{fn: getNextPage}
}
