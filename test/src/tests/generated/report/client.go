...
func (client BaseClient) GetOptionalReport(ctx context.Context, qualifier string) (result SetInt32, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/BaseClient.GetOptionalReport")
		defer func() {
			sc := -1
	...
	req, err := client.GetOptionalReportPreparer(ctx, qualifier)
	if err != nil {
		err = autorest.NewErrorWithError(err, "report.BaseClient", "GetOptionalReport", nil, "Failure preparing request")
		return
	}

	resp, err := client.GetOptionalReportSender(req)
	if err != nil {
		result.Response = autorest.Response{Response: resp}
		err = autorest.NewErrorWithError(err, "report.BaseClient", "GetOptionalReport", resp, "Failure sending request")
		return
	}

	result, err = client.GetOptionalReportResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "report.BaseClient", "GetOptionalReport", resp, "Failure responding to request")
	}

	return
}

// GetOptionalReportPreparer prepares the GetOptionalReport request.
func (client BaseClient) GetOptionalReportPreparer(ctx context.Context, qualifier string) (*http.Request, error) {
	...
	preparer := autorest.CreatePreparer(
		...
		autorest.WithPath("/report/optional"),
		autorest.WithQueryParameters(queryParameters))
	return preparer.Prepare((&http.Request{}).WithContext(ctx))
}

// GetOptionalReportSender sends the GetOptionalReport request. The method will close the
// http.Response Body if it receives an error.
func (client BaseClient) GetOptionalReportSender(req *http.Request) (*http.Response, error) {
	return client.Send(req, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
}

// GetOptionalReportResponder handles the response to the GetOptionalReport request. The method always
// closes the http.Response Body.
func (client BaseClient) GetOptionalReportResponder(resp *http.Response) (result SetInt32, err error) {
