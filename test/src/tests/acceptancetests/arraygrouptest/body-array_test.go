...
func (s *ArrayGroupSuite) TestGetArrayItemEmpty(c *chk.C) {
	...
	c.Assert(err, chk.IsNil)
	c.Assert(res.Items, chk.DeepEquals, [][]string{{"1", "2", "3"},
		{},
		{"7", "8", "9"}})
}
...
func (s *ArrayGroupSuite) TestGetEmptyArray(c *chk.C) {
	...
	c.Assert(res.Items, chk.DeepEquals, []int32{})
}
...
func (s *ArrayGroupSuite) TestGetBooleanInvalidNull(c *chk.C) {
	...
	c.Assert(err, chk.IsNil)
	c.Assert(res.Items, chk.DeepEquals, []bool{true, false, false})
}
...
func (s *ArrayGroupSuite) TestGetByteValidArray(c *chk.C) {
	...
	c.Assert(err, chk.IsNil)
	c.Assert(res.Items, chk.DeepEquals, [][]byte{{255, 255, 255, 250},
		{1, 2, 3},
		{37, 41, 67}})
}
...
// 	c.Assert(err, chk.IsNil)
// 	c.Assert(res.Items, chk.HasLen, 3)
// 	c.Assert(res.Items, chk.DeepEquals, []string{
...
func (s *ArrayGroupSuite) TestGetComplexValidArray(c *chk.C) {
	...
	c.Assert(err, chk.IsNil)
	c.Assert(res.Items, chk.DeepEquals, getProductArray("valid"))
}
...
/*func (s *ArrayGroupSuite) TestGetDateTimeValid(c *chk.C) {
	...
	c.Assert(res.Items, chk.DeepEquals, []date.Time{date.Time{v1}, date.Time{v2}, date.Time{v3}})
}*/
...
/*func (s *ArrayGroupSuite) TestGetDateTimeRfc1123ValidArray(c *chk.C) {
	...
	c.Assert(res.Items, chk.DeepEquals, []date.TimeRFC1123{
		date.TimeRFC1123{v1},
		date.TimeRFC1123{v2},
		date.TimeRFC1123{v3},
	})
...
/*func (s *ArrayGroupSuite) TestGetDateValid(c *chk.C) {
	...
	c.Assert(res.Items, chk.DeepEquals, dateArray)
}*/
...
func (s *ArrayGroupSuite) TestGetDoubleInvalidNull(c *chk.C) {
	...
	c.Assert(res.Items, chk.DeepEquals, []float64{0, 0, -1.2e20})
}
...
func (s *ArrayGroupSuite) TestGetDoubleValid(c *chk.C) {
	...
	c.Assert(err, chk.IsNil)
	c.Assert(res.Items, chk.DeepEquals, []float64{0, -0.01, -1.2e20})
}

func (s *ArrayGroupSuite) TestGetDurationValid(c *chk.C) {
	res, err := arrayClient.GetDurationValid(context.Background())
	c.Assert(err, chk.IsNil)
	c.Assert(res.Items, chk.DeepEquals, []string{"P123DT22H14M12.011S", "P5DT1H0M0S"})
}
...
func (s *ArrayGroupSuite) TestGetIntegerValid(c *chk.C) {
	...
	c.Assert(err, chk.IsNil)
	c.Assert(res.Items, chk.DeepEquals, []int32{1, -1, 3, 300})
}
...
func (s *ArrayGroupSuite) TestGetIntInvalidNull(c *chk.C) {
	...
	c.Assert(res.Items, chk.DeepEquals, []int32{1, 0, 0})
}
...
func (s *ArrayGroupSuite) TestGetLongInvalidNull(c *chk.C) {
	...
	c.Assert(err, chk.IsNil)
	c.Assert(res.Items, chk.DeepEquals, []int64{1, 0, 0})
}
...
func (s *ArrayGroupSuite) TestPutDateTimeValid(c *chk.C) {
	...
	_, err := arrayClient.PutDateTimeValid(context.Background(), []time.Time{
		t1,
		t2,
		t3,
	})
...
/*func (s *ArrayGroupSuite) TestPutDateTimeRfc1123ValidArray(c *chk.C) {
	...
	_, err := arrayClient.PutDateTimeRfc1123Valid(context.Background(), []time.Time{
		t1,
		t2,
		t3,
	})
...
func (s *ArrayGroupSuite) TestGetUUIDValid(c *chk.C) {
	...
	c.Assert(err, chk.IsNil)
	c.Assert(res.Items, chk.DeepEquals, getUUID())
}
...
func getProductArray(str string) []arraygroup.Product {
	var i, j, k int32 = 1, 3, 5
	x, y, z := "2", "4", "6"
	var complexArray []arraygroup.Product
	switch str {
	case "valid":
		complexArray = []arraygroup.Product{
			{&i, &x},
			{&j, &y},
			{&k, &z},
		}
