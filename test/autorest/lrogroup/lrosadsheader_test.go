...
func TestLROSADSBeginPutAsyncRelativeRetry400(t *testing.T) {
	...
	rt, err := poller.ResumeToken()
	require.NoError(t, err)
	poller, err = op.BeginPutAsyncRelativeRetry400(context.Background(), &LROSADsClientBeginPutAsyncRelativeRetry400Options{
		ResumeToken: rt,
	})
	require.NoError(t, err)
	_, err = poller.PollUntilDone(context.Background(), &runtime.PollUntilDoneOptions{Frequency: time.Second})
...
func TestLROSADSBeginPutAsyncRelativeRetryInvalidJSONPolling(t *testing.T) {
	...
	rt, err := poller.ResumeToken()
	require.NoError(t, err)
	poller, err = op.BeginPutAsyncRelativeRetryInvalidJSONPolling(context.Background(), &LROSADsClientBeginPutAsyncRelativeRetryInvalidJSONPollingOptions{
		ResumeToken: rt,
	})
	require.NoError(t, err)
	_, err = poller.PollUntilDone(context.Background(), &runtime.PollUntilDoneOptions{Frequency: time.Second})
...
func TestLROSADSBeginPutAsyncRelativeRetryNoStatus(t *testing.T) {
	...
	rt, err := poller.ResumeToken()
	require.NoError(t, err)
	poller, err = op.BeginPutAsyncRelativeRetryNoStatus(context.Background(), &LROSADsClientBeginPutAsyncRelativeRetryNoStatusOptions{
		ResumeToken: rt,
	})
	require.NoError(t, err)
	_, err = poller.PollUntilDone(context.Background(), &runtime.PollUntilDoneOptions{Frequency: time.Second})
...
func TestLROSADSBeginPutAsyncRelativeRetryNoStatusPayload(t *testing.T) {
	...
	rt, err := poller.ResumeToken()
	require.NoError(t, err)
	poller, err = op.BeginPutAsyncRelativeRetryNoStatusPayload(context.Background(), &LROSADsClientBeginPutAsyncRelativeRetryNoStatusPayloadOptions{
		ResumeToken: rt,
	})
	require.NoError(t, err)
	_, err = poller.PollUntilDone(context.Background(), &runtime.PollUntilDoneOptions{Frequency: time.Second})
...
func TestLROSADSBeginPutError201NoProvisioningStatePayload(t *testing.T) {
	...
	rt, err := poller.ResumeToken()
	require.NoError(t, err)
	poller, err = op.BeginPutError201NoProvisioningStatePayload(context.Background(), &LROSADsClientBeginPutError201NoProvisioningStatePayloadOptions{
		ResumeToken: rt,
	})
	require.NoError(t, err)
	_, err = poller.PollUntilDone(context.Background(), &runtime.PollUntilDoneOptions{Frequency: time.Second})
...
func TestLROSADSBeginPutNonRetry201Creating400(t *testing.T) {
	...
	rt, err := poller.ResumeToken()
	require.NoError(t, err)
	poller, err = op.BeginPutNonRetry201Creating400(context.Background(), &LROSADsClientBeginPutNonRetry201Creating400Options{
		ResumeToken: rt,
	})
	require.NoError(t, err)
	_, err = poller.PollUntilDone(context.Background(), &runtime.PollUntilDoneOptions{Frequency: time.Second})
...
func TestLROSADSBeginPutNonRetry201Creating400InvalidJSON(t *testing.T) {
	...
	rt, err := poller.ResumeToken()
	require.NoError(t, err)
	poller, err = op.BeginPutNonRetry201Creating400InvalidJSON(context.Background(), &LROSADsClientBeginPutNonRetry201Creating400InvalidJSONOptions{
		ResumeToken: rt,
	})
	require.NoError(t, err)
	_, err = poller.PollUntilDone(context.Background(), &runtime.PollUntilDoneOptions{Frequency: time.Second})
